...
import { recursiveCloneChildren } from "@ui/components/recursive-clone-children";
...
export const buttonVariants = tv({
  slots: {
    root: [
      // base
      "group relative inline-flex items-center justify-center whitespace-nowrap outline-none text-label-sm",
...
const ButtonRoot = React.forwardRef<HTMLButtonElement, ButtonRootProps>(
  (
    ...
    if (asChild) {
      // Use JSX directly with type assertion to avoid React 19 type issues
      const SlotComponent = Slot as any;
      return (
        <SlotComponent ref={forwardedRef} className={rootClassName} {...rest}>
          {extendedChildren}
        </SlotComponent>
      );
    }
